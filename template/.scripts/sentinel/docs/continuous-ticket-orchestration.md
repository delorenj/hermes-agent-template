...
fields with a fixed system `PATH`, credentials and endpoints for the prepared
provider, the complete
descriptor-bound provider configuration, and scalar configuration values under
the `HERMES_PROVIDER_CONFIG_` namespace. A controller-only environment field is
never inherited, credentials are never printed, and `TICKET_PROVIDER` remains
the prepared provider. Bubblewrap mounts `/` read-only, then mounts only the
identity-checked prepared repository and one fresh dedicated temporary
directory writable. `TMPDIR` names that temporary directory; it is removed only
after the complete provider subtree is terminated and reaped. No persistent
path outside the prepared repository is writable.

A detached trusted supervisor—not the delivery controller—owns the keyed socket
for the complete provider lifetime, so
