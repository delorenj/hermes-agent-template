...
pidfd is used for bounded terminal cleanup, and failure to prove containment
completion fails closed before finalization or socket release. Unsupported
...
Plane and Trello `resolve_issue_id` HTTP bodies go first to bounded private
...
numeric identities fail closed. Plane uses the supported
